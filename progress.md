...
- [x] TCP-mode hop probing (`--tcp-hop-probe auto|on|off`, `--tcp-hop-probe-port`, default 443): forward + reverse SYN-with-TTL TTL-walker complementing the ICMP prober. Linux/macOS path uses kernel-managed `connect()` with `IP_TTL` + `SO_LINGER=0` to avoid TIME_WAIT, demuxes ICMP `TimeExceeded` on a dedicated raw socket by embedded TCP src-port. Windows is a degrade-silently stub (`Supported=false`). Cross-source correlator: `agg.CorrelateTCP` scores TCP hops against the same stream-loss buckets, `agg.MergeSuspects` upgrades co-flagged TTLs to evidence "RTT/loss correlated in both ICMP and TCP probes". JSON `tcp_hop_probe` + `correlation[].source` sections; text renderer + TUI tables render parallel to ICMP. Server-side reverse TCP-hop gated on the same `privcheck.RawICMP()` capability as ICMP reverse trace. Wire fields all omitempty so old client/server combos stay byte-identical when the feature is off
...
- [x] TCP-mode hop prober (`TCPProber`, Linux/macOS in `probe_tcp_unix.go`, Windows stub in `probe_tcp_windows.go`) — sends SYN per TTL via kernel `connect()+IP_TTL+SO_LINGER=0`, demuxes ICMP `TimeExceeded` on a dedicated raw ICMP socket by embedded TCP src-port (`parse_tcp.go`). Per-prober reserved source-port range avoids cross-prober collisions; reuses `hopState`/`recordReply`/`buildSnapshot` from the shared probe surface. Forward path on client; reverse path on server (gated on `privcheck.RawICMP()`)
