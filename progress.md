...
- [ ] CI — pipeline not yet wired in the public repo. `make all` + `make e2e` cover the local build and smoke loop until a CI provider is chosen
...
- [~] Release/distribution mechanism (spec § DoD #5) — `make all` stamps the binary's version via `-ldflags "-X main.version=$(VERSION)"`, defaulting to `git describe --tags --always --dirty` so a tagged build prints e.g. `v1.0.0` and an untagged commit prints e.g. `aed0b2f-dirty`. A CI-driven release channel (tag-triggered build that publishes binaries + `SHA256SUMS`) is still TODO
